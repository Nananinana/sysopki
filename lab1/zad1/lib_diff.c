...
Diff_handler *diff_handler;   //tablica glowna

int create_table(unsigned int size)
{
    diff_handler = calloc(1, sizeof(diff_handler));  //alokujemy tablice glowna
    diff_handler->size = size; 
    diff_handler->block_array = calloc(size, sizeof(Operations *)); //ustawiamy wskaznik na blok operacji edycyjnych, ktory juz jest odpowiedniego rozmiaru
    return 1;
}

int get_file_size(FILE *fp)  //znajduje rozmiar pliku, nie zdefiniowane w pliku naglowkowym bo nie jest potrzebne "na zewnatrz"
{
    fseek(fp, 0L, SEEK_END);  //ustawia pointer na koniec pliku (ustawia na seek_end i stamtąd przesuwa o 0)
    int sz = ftell(fp);      //zwraca aktualna pozycje pointera
    rewind(fp);    //to samo co fseek(fp, 0,0)
    return sz;
}
...
int diff_files(char *file1, char *file2)  //porownuje pliki i zwraca plik tymczasowy
{
    char buf[256];
    snprintf(buf, sizeof buf, "diff %s %s > %s", file1, file2, tmp_file);  //snprintf zapisuje stringa do bufora, w formacie diff plik jeden i plik2 > plik wynikowy, s to po prostu lancuch znakow
    int find_status = system(buf);   //powoduje wywolanie w powloce (linuxowej) odpowiedniego polecenia, w tym przypadku polecenia porownania i zapisania do pliku wynikowego
    if (find_status != 0)
    {
...
{
    char *stack[size]; //tablica wskaznikow (stos??) podanej wielkosci

    char *p = buff; //wskaznik p na bufor ktory dostalismy

    int i = 0;
    stack[i] = p; //ustawiamy pierwszy wskazik z tablicy (stosu) na pierwszy element bufora(?)
 
    while (*p != '\0') //znak null
    {

        if (*p == '\n' && (*(++p) > '0' && *(p) < '9'))  //n-przejscie do nastepnej linii, ???
        {
            i++;
            stack[i] = p;   //to przesuwamy sie do nastepego elementu tablicy (stosu
        }
        p++;  //a jesli nie, to tylko przesuwamy sie po buforze
    }
//czyli to dzieli bufor na kawalki na ktore wskazniki sa kolejno przypisywane kolejnym komorkom w tablicy (stosie)
